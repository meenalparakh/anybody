...
export ISAACLAB_PATH=${PROJECT_PATH}/isaaclab
...
extract_isaacsim_path() {
    ...
    local isaac_path=${PROJECT_PATH}/_isaac_sim
    ...
    if [ ! -d "${isaac_path}" ]; then
        ...
        echo -e "\t3. Isaac Sim directory is not available at the default path: ${PROJECT_PATH}/_isaac_sim" >&2
        # exit the script
        exit 1
    fi
    # return the result
...
extract_python_exe() {
    ...
    if ! [[ -z "${CONDA_PREFIX}" ]]; then
        ...
        # use kit python
        local python_exe=${PROJECT_PATH}/_isaac_sim/python.sh
    ...
    if [ ! -f "${python_exe}" ]; then
        ...
        echo -e "\t1. Conda environment is not activated." >&2
        echo -e "\t2. Isaac Sim pip package 'isaacsim-rl' is not installed." >&2
        echo -e "\t3. Python executable is not available at the default path: ${PROJECT_PATH}/_isaac_sim/python.sh" >&2
        exit 1
    fi
    # return the result
    echo ${python_exe}
...
setup_conda_env() {
    ...
    local isaacsim_setup_conda_env_script=${PROJECT_PATH}/_isaac_sim/setup_conda_env.sh
...
update_vscode_settings() {
    ...
    setup_vscode_script="${PROJECT_PATH}/.vscode/tools/setup_vscode.py"
...
while [[ $# -gt 0 ]]; do
    ...
    case "$1" in
        -i|--install)
            ...
            ${python_exe} -m pip install -e ${PROJECT_PATH}
        ...
        -c|--conda)
            ...
            if [ -z "$2" ]; then
                echo "[INFO] Using default conda environment name: anybody"
                conda_env_name="anybody"
            else

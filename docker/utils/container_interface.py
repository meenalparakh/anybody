...
import sys
sys.path.append(os.environ['ISAACLAB_PATH'] + "/docker/utils/container_interface.py")
...
class ContainerInterface:
    ...
    def start(self):
        ...
        subprocess.run(
            ...
            + ["--env-file", ".env.base"]
            + ["--env-file", f".env.{self.profile}"]
            + ["up", "--detach", "--build", "--remove-orphans", "isaac-lab-anybody"],
            check=False,
            cwd=self.context_dir,
            env=self.environ,
        )
    ...
    def copy(self, output_dir: Path | None = None):
        ...
        if self.is_container_running():
            ...
            docker_isaac_lab_path = Path(self.dot_vars["DOCKER_PROJECT_PATH"])
            artifacts = {

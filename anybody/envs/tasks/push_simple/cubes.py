...
def get_all_obstacle_pos(robo_dict, obj_dict, z_ground, obstacle_dict=None, flag="init"):
    max_tries = 100
    pose_dict = {}
    ...
    obj_pos_bounds_init = np.array([[-0.5, -0.3], [-0.3, 0.3]])
    obj_pos_bounds_goal = np.array([[0.3, -0.3], [0.5, 0.3]])

    if flag == "init":
        obj_pos_bounds = obj_pos_bounds_init
    else:
        obj_pos_bounds = obj_pos_bounds_goal
...
def get_base_env(robo_type, n_obstacles, n_obs, robo_fname, seed=0, version_name="v1"):
    np.random.seed(seed)
    random.seed(seed)
    ...
    while True:
        ...
        init_pose_dict = get_all_obstacle_pos(
            robo_dict, obj_dict, z_ground=z_ground, obstacle_dict=static_obs_dict, flag="init"
        )
        goal_pose_dict = get_all_obstacle_pos(
            robo_dict, obj_dict, z_ground=z_ground, obstacle_dict=static_obs_dict, flag="goal"
        )
    ...
    def obj_sampler(_obj_pos_bounds_init, _obj_pos_bounds_goal, flag="init"):
        if flag == "init":
            p = np.eye(4)
            p[:2, 3] = U(_obj_pos_bounds_init[0], _obj_pos_bounds_init[1])
            p[:3, :3] = R.from_euler("xyz", [0, 0, U(-np.pi, np.pi)]).as_matrix()
            return p
        
        else:
            p = np.eye(4)
            p[:2, 3] = U(_obj_pos_bounds_goal[0], _obj_pos_bounds_goal[1])
            p[:3, :3] = R.from_euler("xyz", [0, 0, U(-np.pi, np.pi)]).as_matrix()
            return p
    ...
    def get_cfg_fn():
        return eu.get_randomized_push_simple_cfg(
            ...
            obj_sampler_args=[obj_pos_bounds_init, obj_pos_bounds_goal],
...
def get_v1_env(robo_type, robo_fname, seed=0):
    return get_base_env(robo_type, n_obstacles=1, n_obs=1, robo_fname=robo_fname, seed=seed)
...
def get_v2_env(robo_type, robo_fname, seed=0):
    return get_base_env(robo_type, n_obstacles=4, n_obs=2, robo_fname=robo_fname, seed=seed)
...
for cube_fname in cube_fnames:
    ...
    cube_joint_envs[f'{cube_fname}_v2'] = (get_v2_env, 'cubes', cube_fname)
    cube_pose_envs[f'{cube_fname}_v2'] = (get_v2_pose_env, 'cubes', cube_fname)

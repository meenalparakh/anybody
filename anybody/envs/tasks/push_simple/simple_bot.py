...
def get_base_env(robo_type, robo_fname, n_obs, n_blocks, seed=0, version_name="v1"):
    ...
    robot = eu.get_robot(robo_type=robo_type, robo_fname=robo_fname, robo_id=0)
    ...
    for _ in range(max_tries):
        ...
        block_pos_bounds_init = np.array([[init_x_range[0], 0.1, 0.0], [init_x_range[1], 0.5, 1.0]])
        block_pos_bounds_goal = np.array([[0.3, 0.1, 0.0], [0.5, 0.5, 1.0]])
...
def get_v1_env(robo_type, robo_name, seed=0):
    return get_base_env(robo_type, robo_name, 0, 1, seed, version_name="v1")


def get_v2_env(robo_type, robo_name, seed=0):
    n_blocks = np.random.randint(1, 5)
    return get_base_env(robo_type, robo_name, 0, n_blocks=n_blocks, seed=seed, version_name="v2")


def get_v3_env(robo_type, robo_name, seed=0):
    n_obs = np.random.randint(1, 10)
    n_blocks = np.random.randint(1, 5)
    return get_base_env(robo_type, robo_name, n_obs, n_blocks, seed=seed, version_name="v3")

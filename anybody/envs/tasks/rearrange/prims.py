...
# in general rearrange - we need to add all the variations
# grasp, push, rotate, reach (automatically included)
# the base is from grasp - so already present

# we need to add push and rotate
...
def get_robot(idx=0, robo_id=0):
    ...
    jvals_low = [-1, -1.0, 0.2, -np.pi, -np.pi]
    jvals_high = [1, 1.0, 0.7, np.pi, np.pi]
...
def add_drawer_to_env(ground_z, obs_dict, art_type="drawer"):
    rot_variation = True

    found = False
    from anybody.articulation_utils import load

    def get_random_door():
        d_pos = [U(-0.3, 0.3), U(-0.3, 0.3), U(0.1, 0.2)]
        ...
        door, ht = load.get_door(door_idx, pos=d_pos, rotz=rotz, rotx=rotx)
        door.pose[2, 3] = ground_z + ht + U(0.3, 0.5)
        return door

    def get_random_turner():
        d_pos = [U(-0.3, 0.3), U(-0.3, 0.3), 0]
        ...
        turner, ht = load.get_turner(turner_idx, pos=d_pos, rotz=rotz)
        turner.pose[2, 3] = ground_z + U(0.05, 0.15) + ht
        return turner

    def get_random_drawer():
        d_pos = [U(-0.3, 0.3), U(-0.3, 0.3), 0]
        ...
        drawer, ht = load.get_drawer(drawer_idx, pos=d_pos, rotz=rotz)
        drawer.pose[2, 3] = ground_z + ht + 0.001
        return drawer

    for _ in range(100):
        if art_type == "door":
            d = get_random_door()
        elif art_type == "turner":
            d = get_random_turner()
        else:
            d = get_random_drawer()

        d_joint_names = d.act_info["joint_names"]
        drawer_joint_lb = d.act_info["joint_lb"]
        drawer_joint_ub = d.act_info["joint_ub"]

        init_d_jpos = dict(zip(d_joint_names, U(drawer_joint_lb, drawer_joint_ub)))

        is_collision = eu.check_collision(d, init_d_jpos, obs_dict, view=False)
        if not is_collision:
            goal_d_jpos = dict(zip(d_joint_names, U(drawer_joint_lb, drawer_joint_ub)))
            is_collision = eu.check_collision(d, goal_d_jpos, obs_dict, view=False)
            if not is_collision:
                found = True
                break
    ...
    return d, init_d_jpos, goal_d_jpos


def get_base_env(n_obs, n_blocks, seed=0, art_type="drawer"):
    np.random.seed(seed)
    random.seed(seed)
    ...
    for _ in range(max_tries):
        ...
        obst_pos_bounds = np.array([[-0.5, -0.5, -0.2], [0.5, 0.5, 0.5]])
        ...
        drawer, init_d_jpos, goal_d_jpos = add_drawer_to_env(
            z_ground, obstacle_dict, art_type=art_type
        )
        if drawer is not None:
            assert init_d_jpos is not None
            drawer.joint_pos = init_d_jpos
            obstacle_dict[f"{art_type}_{drawer.articulation_id}"] = drawer
        ...
        all_obs["ground"] = eu.Prim(
            obj_type="box",
            obj_id=n_obs + n_blocks + 2,
            obj_shape=[2, 2, 0.01],
            pose=p,
            static=True,
        )
        ...
        # find a target block positions
        target_block_pos = {}
        if drawer is not None:
            assert goal_d_jpos is not None
            drawer.joint_pos = goal_d_jpos
    ...
    art_goal_dict = {}
    if drawer is not None:
        all_obs.pop(f"{art_type}_{drawer.articulation_id}")
        articulation_dict = {drawer.articulation_id: drawer}
        art_init_dict = {
    ...
    return eu.ProblemSpec(
        ...
        init_articulation_state_dict=art_init_dict,
        goal_articulation_state_dict=art_goal_dict,
        ground=z_ground,
    )
...
def get_v4_env(seed=0):
    n_obs = np.random.randint(1, 5)
    return get_base_env(n_obs, 1, seed, art_type="turner")


def get_v5_env(seed=0):
    n_obs = np.random.randint(1, 5)
    return get_base_env(n_obs, 1, seed, art_type="door")

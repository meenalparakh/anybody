...
def get_flap_link(flap_length, flap_thickness, width, cylinder_radius, margin):
...
def get_urdf(width=0.01, flap_length=0.05, flap_thickness=0.001, bend_theta=np.pi/6, flap_length2=0.1):
...
"""
    ...
    <joint name="joint_FLAPNAME" type="fixed">   
        <parent link="PARENT"/>
        <child link="FLAPNAME"/>
        <origin xyz="{2 * cylinder_radius + flap_length + 2 * margin} 0 0" rpy="0 0 THETA"/>
    </joint>
"""

    flap_link = get_flap_link(flap_length, flap_thickness, width, cylinder_radius, margin)
    ...
    flap_link2 = get_flap_link(flap_length2, flap_thickness, width, cylinder_radius, margin)
    
    flap_left2 = format_str(flap_link2 + flap_joint2, {
        'FLAPNAME': 'flap_left2',
        'PARENT': 'flap_left1',
        'THETA': str(-2 * bend_theta)
    })
    
    flap_right2 = format_str(flap_link2 + flap_joint2, {
...
def save_urdfs(to_usd=False):
    ...
    lenghts2 = np.linspace(0.1, 0.05, 3)
    ...
    for l_idx, (l1, l2) in enumerate(zip(lengths1, lenghts2)):

        robo_name = f"tongs_{l_idx}"
        robo_dir = get_robot_morphs_dir() / "tongs_v2" / robo_name

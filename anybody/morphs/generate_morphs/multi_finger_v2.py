...
def get_joint_link(
    parent_link, child_link, origin, rpy, axis, joint_ll, joint_ul, joint_effort, joint_vel
):
    joint_link = f"""
...
# def get_base_joint(parent_link, child_link, origin, rpy):
#     joint_link = f"""
#     <joint name="{child_link}_joint" type="revolute">
#         <origin xyz="{origin[0]} {origin[1]} {origin[2]}" rpy="{rpy[0]} {rpy[1]} {rpy[2]}"/>
#         <parent link="{parent_link}"/>
...
def get_single_flink(linkname, length, width, thickness, radius):
    ...
    <link name="{linkname}">
        ...
        <collision>
            <origin xyz="{radius + length/2} 0 0" rpy="0 0 0"/>
            <geometry>
                <box size="{length * collision_ratio} {width} {thickness}"/>
            </geometry>
        </collision>
    </link>
"""
...
def get_finger(
    ...
    parent_linkname = "cylindrical_baselink"
    origin = [0, 0, 0]
    ...
    xyz = np.array([xy[0], xy[1], 0]) + base_offset
    ...
    joint = get_joint_link(
        ...
        rpy=[0, 0, yaw],
        axis=[0, 1, 0],
        joint_ll=joint_lower_limits[0],
        joint_ul=joint_upper_limits[0],
        joint_effort=1,
        joint_vel=1,
    )
...
def get_hand_urdf(n_fingers, per_finger_lengths, equally_distanced=True, finger_params={}, base_link_origin=[0, 0, 0], base_radius=0.02, length=0.005):
    
    ox, oy, oz = base_link_origin

    base_cylindrical_link = f"""
    <link name="cylindrical_baselink">
        <inertial>
        ...
        <visual>
            <origin xyz="{ox} {oy} {oz}" rpy="0 0 0"/>
            <geometry>
                <cylinder radius="{base_radius}" length="{length}"/>
            </geometry>        
        </visual>
        <collision>
            <origin xyz="{ox} {oy} {oz}" rpy="0 0 0"/>
...
"""
    ...
    for idx, per_f_length in enumerate(per_finger_lengths):
        finger1 = get_finger(
            ...
            yaw=thetas[idx],
            base_offset=base_link_origin,
            **finger_params,
        )
        all_fingers += finger1

    return base_cylindrical_link + all_fingers, "cylindrical_baselink"
...
"""
    ...
    hand, base_name = get_hand_urdf(n_fingers, per_finger_lengths=per_finger_lengths, 
                                    equally_distanced=equally_distanced, finger_params=finger_params)

    dof5_link = get_5dof_base_link(base_name, pos_only=True)
    ...
    for n in n_fingers:
        for eq_idx, eq in enumerate(equidistant):
            for fl_idx, fl in enumerate([finger_link_lengths1, finger_link_lengths2]):
                ...
                robo_dir = get_robot_morphs_dir() / "mf_v2" / robo_name

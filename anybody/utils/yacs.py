...
def _check_and_coerce_cfg_value_type(replacement, original, key, full_key):
    ...
    if full_key.startswith('MULTIENV.') or full_key.startswith('TEST_ENVS.'):
        # the MULTIENV and TEST_ENVS keys are handled by the train script
        if isinstance(replacement, (str, int)) and isinstance(original, (list, tuple)):
            return replacement
        if isinstance(replacement, (list, tuple)) and isinstance(original, (str, int)):
            return replacement
